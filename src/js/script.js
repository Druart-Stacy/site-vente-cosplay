...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (articleTitle) articleTitle.textContent = alt;
  if (articlePrice) articlePrice.textContent = price;
  if (articleAvailability) articleAvailability.textContent = `Disponibilité : ${availability}`;
  ...
  links.forEach(link => {
      link.addEventListener('click', event => {
          ...
          if (simageElement) simageElement.src = newImageSrc; // Met à jour l'image affichée
      });
  });
});
